Of course. I've analyzed your `ChatSection.tsx` component and identified the two issues you're facing.

1.  **First Message Not Appearing Immediately:** This is a classic React Query optimistic update problem. The issue is in how you handle the transition from a "new chat" (where `currentConversationId` is `null`) to an "existing chat" (once the first message is sent and a `conversationId` is created). Your `onMutate` updates one query key, but your `onSuccess` logic mishandles the switch to the new query key, causing the optimistic update to be lost.

2.  **"New Chat" Button Not Functioning:** The button correctly sets `currentConversationId` to `null`, but the way your `useQuery` is structured with a dynamic key (`['/api/conversations', currentConversationId, 'messages']`) and the way you update the cache (`queryClient.setQueryData(['/api/messages'], ...)`), the query doesn't get the updated data because the keys don't match.

Here is the corrected and improved version of your `ChatSection.tsx` component. I've added comments `// FIX:` and `// CHANGE:` to highlight the important modifications.

### Corrected `ChatSection.tsx`
...
// FIX 1: Define a static welcome message to be reused.
...
const ChatSection: React.FC = () => {
  ...
  // CHANGE 1: Create a consistent query key based on the conversation ID.
  // This makes it easier to manage the cache for new vs. existing conversations.
  ...
  const { data: messages, isLoading: loadingMessages } = useQuery({
    // CHANGE 2: Use the consistent query key.
    queryKey: messagesQueryKey,
    queryFn: async () => {
      // FIX 2: Handle the "new chat" case directly instead of a separate API call.
      // This makes the "New Chat" button instant.
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: (data, variables, context) => {
      // FIX 3: This is the core fix for the first message display.
      // We now handle the transition from a 'new' chat to a saved one.
  ...
  // FIX 4: Simplified and corrected "New Chat" handler.
  const handleNewChat = () => {
    ...
    // though changing the key should be sufficient.
...
1.  **Consistent Query Keys:** I introduced a `messagesQueryKey` variable that resolves to `['messages', 'new']` for new chats and `['messages', 'some-id']` for existing ones. This is the foundation for fixing both issues, as it removes ambiguity about which cache key to read from or write to.

2.  **Instant "New Chat":** The `useQuery` now checks if `currentConversationId` is null. If it is, it instantly returns a static `welcomeMessage` instead of making a network request. This makes the "New Chat" button feel immediate.

3.  **Simplified `handleNewChat`:** This function is now much simpler. It just needs to set `currentConversationId` to `null`. React Query automatically detects the key change and uses the logic from point #2 to display the welcome message.

4.  **Corrected `onSuccess` Logic:** This is the most critical fix.
    *   It now checks if the message was the *first* in a conversation (`isFirstMessage`).
    *   If so, it performs a seamless transition:
        1.  It reads the optimistically updated messages from the `['messages', 'new']` cache.
        2.  It creates the new messages array using the real data from the server response.
        3.  It uses `queryClient.setQueryData` to **pre-populate the cache** for the new conversation ID (`['messages', newConversationId]`).
        4.  It **then** calls `setCurrentConversationId(newConversationId)`.
    *   This sequence ensures that when the component re-renders and asks for the new conversation's data, it's already in the cache, and the transition is invisible to the user. Your optimistically added message is preserved throughout this process. Subsequent messages in the same chat continue to work as they did before.