...
export interface MetricsVisibilitySettings {
  visible_categories: string[];
  hidden_categories: string[];
  dashboard_preferences: {
    visible_metrics: string[];
...
export const useHealthVisibilitySettings = () => {
  // Get current visibility settings
  const { data: visibilitySettings, isLoading, error } = useQuery({
  ...
  return {
    ...
    isLoading,
    error,
    updateSettings: updateVisibilityMutation.mutateAsync,
    isUpdating: updateVisibilityMutation.isPending,
  };
};
